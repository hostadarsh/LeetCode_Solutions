...
class Solution {
    public int maxDiff(int num) {
        ...
        for(char ch:s.toCharArray()){
            String maxs = s.replace(ch, '9');
            String mins = s.replace(ch, '0');
            if(ch==first)mins=s.replace(ch, '1');
            int tmax = Integer.valueOf(maxs);
            int tmin = Integer.valueOf(mins);
            max = Math.max(max, tmax);
            if(tmin!=0)min = Math.min(min, tmin);
        }
